...
def category_products(request, category_id):
    mongo_db = MongoDB()
    categories_collection = mongo_db.get_collection('categories')
    products_collection = mongo_db.get_collection('products')
    product_images_collection = mongo_db.get_collection('productimage') 
    ...
    for product in products:
        product['images'] = list(product_images_collection.find({"product_id": product['id']}))  
...
def product_detail(request, product_id):
    ...
    similar_product = list(products_collection.find({
        "category": str(product.category.id),
        "product_id": {"$ne": product.id}
    }).limit(5))
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            cart_item.save()
            mongo_db = MongoDB()
            cart_collection = mongo_db.get_collection('cart')
            ...
            mongo_data = {
                ...
                "processor": [str(proc.id) for proc in product.processor.all()],
                "mother": [str(m.id) for m in product.mother.all()],
                "ram": [str(r.id) for r in product.ram.all()],
                "storage": [str(s.id) for s in product.storage.all()],
                "graphics": [str(g.id) for g in product.graphics.all()],
                "operating_system": [str(os.id) for os in product.operating_system.all()],
                "weight": float(product.weight) if product.weight is not None else None,  # Приведение weight к float
                "screen_sizes": [str(size.id) for size in product.screen_sizes.all()],
                "screen_type": [str(st.id) for st in product.screen_type.all()],
                "screen_resolution": [str(sr.id) for sr in product.screen_resolution.all()],
                "touch_screen_touches": [str(ts.id) for ts in product.touch_screen_touches.all()],
                "formfactor": [str(ff.id) for ff in product.formfactor.all()],
                "webcam": [str(wc.id) for wc in product.webcam.all()],
                "keyset": [str(ks.id) for ks in product.keyset.all()],
                "keyboard_backlight": [str(kbl.id) for kbl in product.keyboard_backlight.all()],
                "power_supplies": [str(ps.id) for ps in product.power_supplies.all()],
                "operating_temperature": product.operating_temperature,
                "storage_temperature": product.storage_temperature,
                "operating_humidity": product.operating_humidity,
                "storage_humidity": product.storage_humidity,
                "sizes": [str(sz.id) for sz in product.sizes.all()],
                "controllers": [str(ctrl.id) for ctrl in product.controllers.all()],
            }
            
            # Добавление данных в коллекцию `cart` в MongoDB
            cart_collection.insert_one(mongo_data)
...
def search(request):
    ...
    # Поиск продуктов по названию или описанию в MongoDB
    result_products = list(products_collection.find({
        "$or": [
...
def add_product(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid() and image_formset.is_valid() and description_formset.is_valid():
            ...
            # Добавление продукта в коллекцию products в MongoDB
...
def approve_request(request, request_id):
    ...
    if registration_request.status == 'pending':
        ...
        # Данные пользователя для MongoDB
        user_data = {
            "username": registration_request.email,
            "email": registration_request.email,
            "first_name": registration_request.first_name,
...
def delete_cart_item(request, item_id):
    ...
    # Подключение к MongoDB и выбор коллекции cart
    mongo_db = MongoDB()
    cart_collection = mongo_db.get_collection('cart')
    ...
    cart_collection.delete_one(mongo_data)
    
    # Удаление из базы данных Django
    cart_item.delete()
    mongo_db.close()  # Закрытие соединения с MongoDB
